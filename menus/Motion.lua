-- deobfuscated by DutchCoder
--disabled that gay login shit and chat spam every few seconds promoting there server
    ...
    local logged = true -- disabled login bc thats gay
    local pass = "gay"
    ...
    function Motion.CheckBox(text, checked, callback)
        if Motion.Button(text, checked and '~h~~w~On' or "~h~~c~Off") then
            checked = not checked
        ...
        local SelectedPlayer
            ...
            elseif dA.id ~= 13 and dA.id ~= 12 and Motion.Button(dA.name.." Upgrade 4", price4) then 
                ...
                elseif Motion.IsMenuOpened("InfoMenu") then
                    if Motion.Button("Build 0.9") then
                    end
                ...
                elseif Motion.IsMenuOpened("TrollMenu") then
                    ...
                    elseif Motion.Button("Crash All Players") then
                       runOnAll(crash)
                ...
                elseif Motion.IsMenuOpened("performance") then 
                    ...
                    Motion.Display() elseif Motion.IsMenuOpened("primary") then Motion.MenuButton("∑Classic", "classic1") Motion.MenuButton("∑Metallic", "metallic1") Motion.MenuButton("∑Matte", "matte1") Motion.MenuButton("∑Metal", "metal1") Motion.Display() elseif Motion.IsMenuOpened("secondary") then Motion.MenuButton("∑Classic", "classic2") Motion.MenuButton("∑Metallic", "metallic2") Motion.MenuButton("∑Matte", "matte2") Motion.MenuButton("∑Metal", "metal2") Motion.Display() elseif Motion.IsMenuOpened("rimpaint") then Motion.MenuButton("∑Classic", "classic3") Motion.MenuButton("∑Metallic", "metallic3") Motion.MenuButton("∑Matte", "matte3") Motion.MenuButton("∑Metal", "metal3") Motion.Display() elseif Motion.IsMenuOpened("classic1") then
                    ...
                    bc = false; oldprim, oldsec = GetVehicleColours(veh) oldpearl, oldwheelcolour = GetVehicleExtraColours(veh) b9 = table.pack(oldprim, oldsec, oldpearl, oldwheelcolour) SetVehicleColours(veh, dT.id, oldsec) SetVehicleExtraColours(veh, dT.id, oldwheelcolour) b8 = true elseif b8 and curprim == dT.id then SetVehicleColours(veh, dT.id, oldsec) SetVehicleExtraColours(veh, dT.id, oldwheelcolour) b8 = false; ba = -1; b9 = -1 elseif b8 and curprim ~= dT.id then SetVehicleColours(veh, dT.id, oldsec) SetVehicleExtraColours(veh, dT.id, oldwheelcolour) b8 = true end end end; Motion.Display() elseif Motion.IsMenuOpened("metallic1") then
                    for dS, dT in pairs(bg) do tp, ts = GetVehicleColours(veh) if tp ==
                    dT.id and not b8 then pricetext = "Installed"
                    else if b8 and tp == dT.id then pricetext = "Previewing"
                    else pricetext = "Not Installed"
                ...
                elseif Motion.IsMenuOpened("OnlinePlayersMenu") then
                    ...
                    elseif Motion.IsMenuOpened("PlayerOptionsMenu") then
                        ...
                        elseif Motion.Button("Crash Player") then
